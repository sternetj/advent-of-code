...
(function() {
  ...
  while (!exit) {
    ...
    for (const cart of carts.slice()) {
      ...
      if (currentPositions[currentPos]) {
        let crashedCart = currentPositions[currentPos];
        delete currentPositions[currentPos];
        carts = carts.filter(c => c.id != cart.id && c.id != crashedCart.id);
  ...
  console.log(carts[0].currentSegment.id);
})();
